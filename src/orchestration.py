...
PHASE_DEPENDENCIES = {
    ...
    "train": ("prepare", "tune"),
    "evaluate_raw": ("train",),
    "evaluate_dsp": ("eda", "evaluate_raw"),
    "signal_analysis": ("eda",),
    "statistics": ("evaluate_raw", "evaluate_dsp"),
...
def create_run(
    working_config_path: Path | str, *, project_root: Path | str | None = None
) -> RunContext:
    ...
    manifest: dict[str, Any] = {
        ...
        "source_config_hash": _json_hash(source),
        "data_cache_hash": config.data_cache_hash,
        "status": "running",
        "error": None,
        "phases": {
            name: {
    ...
    *,
    ...
    if _json_hash(source) != _json_hash(bound.config_snapshot_path):
        raise ConfigMismatchError(
            f"{bound.run_tag} uses a different config snapshot; create a new run ID."
        )
...
def preflight_dataset(config: ExperimentConfig) -> dict[str, Any]:
    ...
    if len(speakers) < config.train_speakers + config.validation_speakers + 1:
        raise PreflightError(
            f"VCTK has {len(speakers)} speaker directories; need at least {config.train_speakers + config.validation_speakers + 1}."
        )
    no_wav = [path.name for path in speakers if not _wav_files(path)]
    if no_wav:
        raise PreflightError(
            f"VCTK speakers without WAV recordings: {', '.join(no_wav[:8])}"
        )
    speaker_splits = split_speakers(
        vctk, config.seeds[0], config.train_speakers, config.validation_speakers
    )
    short = []
    for split, selected in speaker_splits.items():
        minimum = (
            ...
            if split == "train"
            else config.eval_clips_per_speaker
        )
        if split == "train" and config.closed_set_clips:
            minimum += config.closed_set_clips
        short.extend(
            f"{p.name} ({len(_wav_files(p))} < {minimum})"
            for p in selected
            if len(_wav_files(p)) < minimum
        )
    if short:
    ...
    for index, family in enumerate(("noise", "music", "speech")):
        try:
            ...
            parts = split_noise_files(
                files, config.seeds[0] + index, config.noise_split_fractions
            )
        except (FileNotFoundError, ValueError) as error:
            raise PreflightError(
                f"MUSAN {family!r} cannot make recording-disjoint pools: {error}"
            ) from error
        families[family] = {
            "recordings": len(files),
            **{name: len(items) for name, items in parts.items()},
        }
    estimated = (
        (
            config.clips_per_speaker * config.train_speakers
            + config.eval_clips_per_speaker * (len(speakers) - config.train_speakers)
            + config.noise_pool_size
        )
    ...
    return {
        "vctk_wav48": str(vctk),
        "vctk_speakers": len(speakers),
        "vctk_speaker_ids": [p.name for p in speakers],
        "musan_root": str(config.musan_root),
        "musan": families,
        "cache_root": str(config.cache_root),
        "estimated_cache_bytes": estimated,
        "estimated_cache_gib": round(estimated / 1024**3, 2),
    }
...
def _phase_prepare(config: ExperimentConfig, workers: int) -> dict[str, Any]:
    ...
    return {
        "inventory": inventory,
        "cache_root": str(config.cache_root),
        "noise_pools": [str(p) for p in pools.values()],
    }


def _phase_eda(config: ExperimentConfig, workers: int) -> dict[str, Any]:
    from src.analysis import derive_band_design
    from src.plots import plot_dsp_design, plot_signal_gallery

    design = derive_band_design(config)
    return {
        "design": design,
        "design_path": str(config.dsp_design_path),
        "figures": [plot_dsp_design(config, design), plot_signal_gallery(config)],
    }


def _phase_tune(config: ExperimentConfig, workers: int) -> dict[str, Any]:
    from src.study import tune_hyperparameters
    from src.plots import plot_tuning_grid

    report = tune_hyperparameters(config, workers=workers)
    return {
        "selected": report["selected"],
        "trials": report["trials"],
        "figure": plot_tuning_grid(config, report),
    }


def _phase_train(config: ExperimentConfig, workers: int) -> dict[str, Any]:
    from src.study import selected_hyperparameters, train_study
    from src.plots import plot_training_histories

    selected = selected_hyperparameters(config)
    results = train_study(
        config,
        workers=workers,
        missing_only=True,
        learning_rate=selected["learning_rate"],
        arcface_margin=selected["arcface_margin"],
    )
    return {
        "hyperparameters": selected,
        "training": [result.__dict__ for result in results],
        "checkpoints": checkpoint_paths(config),
        "figures": plot_training_histories(config),
    }
...
def _phase_evaluate_dsp(config: ExperimentConfig, workers: int) -> dict[str, Any]:
    ...
    return {
        "reports": reports,
        "summary": evaluation_summary(config, ("raw", *cells), "dsp-matched"),
    }
...
def _phase_signal_analysis(config: ExperimentConfig, workers: int) -> dict[str, Any]:
    ...
    return {
        ...
        "characterisation": characterize_frontends(config),
    }
...
PHASE_HANDLERS: dict[str, Callable[[ExperimentConfig, int], dict[str, Any]]] = {
    ...
    "tune": _phase_tune,
    "train": _phase_train,
    "evaluate_raw": _phase_evaluate_raw,
    "evaluate_dsp": _phase_evaluate_dsp,
    "signal_analysis": _phase_signal_analysis,
    "statistics": _phase_statistics,
    "figures": _phase_figures,
}


def _outputs_since(config: ExperimentConfig, before: set[Path]) -> list[str]:
    return sorted(
        str(path.relative_to(config.run_dir))
        for path in config.run_dir.rglob("*")
        if path.is_file() and path not in before and path.name != "run-manifest.json"
    )
...
def checkpoint_paths(config: ExperimentConfig) -> list[str]:
    ...
    if missing:
        raise FileNotFoundError(
            "Expected robust_cnn checkpoints are missing: " + ", ".join(missing)
        )
...
def execute_phase(context: RunContext, phase: str, *, workers: int = 0) -> PhaseResult:
    ...
    if manifest.get("status") == "completed":
        raise RuntimeError(
            f"{config.run_tag} is completed and immutable; allocate a new run."
        )
    unmet = [
        item
        for item in PHASE_DEPENDENCIES[phase]
        if manifest["phases"][item]["status"] != "completed"
    ]
    if unmet:
    ...
    if state["status"] == "completed":
        return PhaseResult(
            phase, "completed", {"skipped": True, "outputs": state["outputs"]}, manifest
        )
    state.update(
        {
            "status": "running",
    ...
    try:
        ...
        state.update(
            {
                ...
                "outputs": _outputs_since(config, before),
            }
        )
        ...
        manifest.update(
            {"status": "failed", "error": {"phase": phase, "message": str(error)}}
        )
