...
class VerificationPair:
    ...
    def pair_id(self) -> str:
        return sha256(
            f"{self.left_sample_id}|{self.right_sample_id}|{int(self.is_same_speaker)}".encode()
        ).hexdigest()[:16]
    ...
    *,
    ...
    for speaker in speakers:
        ...
        for left, right in combinations[
            : min(positive_pairs_per_speaker, len(combinations))
        ]:
    ...
    for index, positive in enumerate(positives):
        source_speaker = next(
            s for s in speakers if positive.left_sample_id.startswith(f"{s}/")
        )
        other_speakers = [speaker for speaker in speakers if speaker != source_speaker]
        other = other_speakers[
            (index + rng.randrange(len(other_speakers))) % len(other_speakers)
        ]
...
def score_pairs(
    pairs: Sequence[VerificationPair], embeddings: Mapping[str, Tensor]
    ...
    for pair in pairs:
        try:
            left, right = (
                embeddings[pair.left_sample_id],
                embeddings[pair.right_sample_id],
            )
        ...
        scores.append(
            functional.cosine_similarity(left.unsqueeze(0), right.unsqueeze(0)).item()
        )
...
def verification_metrics(
    ...
    tn, fp, fn, tp = (
        int(value)
        for value in confusion_matrix(labels, predictions, labels=[0, 1]).ravel()
    )
    ...
    return {
        ...
        "eer": calibration["eer"],
        "eer_threshold": calibration["threshold"],
...
def pair_errors(
    ...
    *,
    threshold: float,
    diagnostics: Mapping[str, Mapping[str, object]] | None = None,
) -> list[dict[str, object]]:
    """Return false positives/negatives with reproducible pair provenance."""
    result: list[dict[str, object]] = []
    predictions = scores >= threshold
    for pair, score, label, prediction in zip(
        pairs, scores, labels, predictions, strict=True
    ):
        if label == prediction:
            continue
        row: dict[str, object] = {
...
def paired_stratified_bootstrap(
    ...
    dsp_scores: np.ndarray,
    labels: np.ndarray,
    *,
    metric: str = "f1",
    threshold_raw: float = 0.0,
    threshold_dsp: float = 0.0,
    seed: int = 11,
    repetitions: int = 2000,
) -> dict[str, float]:
    """Paired, class-stratified bootstrap CI for full-DSP minus raw-noisy."""
    _validate_scores(raw_scores, labels)
    _validate_scores(dsp_scores, labels)
    ...
    for _ in range(repetitions):
        ...
        raw = verification_metrics(
            raw_scores[indices], labels[indices], threshold=threshold_raw
        )[metric]
        dsp = verification_metrics(
            dsp_scores[indices], labels[indices], threshold=threshold_dsp
        )[metric]
...
def clustering_metrics(
    ...
    *,
    ...
    vectors = functional.normalize(
        torch.stack(
            [embeddings[record.sample_id].detach().cpu() for record in ordered]
        ),
        p=2,
        dim=-1,
    ).numpy()
    predicted = hdbscan.HDBSCAN(
    ...
    return {
        ...
        "v_measure": float(v_measure_score(truth, predicted)),
        "clustered_coverage": float((predicted >= 0).mean()),
        "outlier_rate": float((predicted < 0).mean()),
    }
...
def _validate_scores(scores: np.ndarray, labels: np.ndarray) -> None:
    if (
        ...
        raise ValueError(
            "scores/labels must be one-dimensional, equal-sized, and contain both classes"
        )
