...
class AugmentedClips(Dataset):
    ...
    def __init__(
        ...
        self.clips, self.records, self.labels, self.config, self.seed, self.split = (
            clips,
            records,
            labels,
            config,
        ...
        self.epoch = 0
        self.pools = {
            f: load_noise_pool_for_split(config.cache_root, split, f)
            for f in ("noise", "music", "speech")
        }
        self.metadata = {
            f: load_noise_metadata(config.cache_root, split, f) for f in self.pools
        }
    ...
    def augmentation_metadata(self, index: int) -> dict[str, object]:
        ...
        noisy, family, snr = training_augmentation_choice(
            record.sample_id,
            seed=self.seed,
            epoch=self.epoch,
            p_noise=self.config.p_noise,
            families=self.config.train_noise_families,
            snr_range_db=self.config.train_snr_range_db,
        )
        ...
        if noisy:
            data.update(
                make_mixture(
                    ...
                    record.sample_id,
                    seed=self.seed,
                    family=family or "noise",
                    snr_db=float(snr),
                    pools=self.pools,
                    pool_metadata=self.metadata,
                    babble_sources_range=self.config.babble_sources_range,
                    namespace=f"train-epoch-{self.epoch}",
                ).metadata
            )
        return data
    ...
    def __getitem__(self, index: int):
        ...
        return torch.from_numpy(np.asarray(waveform, dtype=np.float32)), torch.tensor(
...
def train_robust_cnn(
    ...
    *,
    ...
    """Overrides exist so the tuning phase searches with this exact recipe."""
    device = device or resolve_device()
    learning_rate = config.learning_rate if learning_rate is None else learning_rate
    arcface_margin = config.arcface_margin if arcface_margin is None else arcface_margin
    epochs = config.epochs if epochs is None else epochs
    seed_everything(seed)
    train_records, validation_records = (
        load_manifest(config.cache_root, "train"),
        load_manifest(config.cache_root, "validation"),
    )
    train_clips, validation_clips = (
        load_clips(config.cache_root, "train"),
        load_clips(config.cache_root, "validation"),
    )
    train_labels, names = speaker_labels(train_records)
    validation_labels, _ = speaker_labels(validation_records)
    train_data = AugmentedClips(
        train_clips, train_records, train_labels, config, seed, "train"
    )
    validation_data = AugmentedClips(
        validation_clips,
        validation_records,
        validation_labels,
        config,
    ...
    head = ArcFaceHead(
        config.embedding_dim, len(names), arcface_margin, config.arcface_scale
    ).to(device)
    ...
    root = config.seed_dir(seed) if root is None else root
    ...
    audit_context = (
        augmentation_path.open("w", encoding="utf-8") if audit else nullcontext(None)
    )
    with audit_context as audit_stream:
        for epoch in range(epochs):
            ...
            if audit_stream is not None:
                for index in range(len(train_data)):
                    audit_stream.write(
                        json.dumps(train_data.augmentation_metadata(index)) + "\n"
                    )
            accuracy = _validation_accuracy(
                model, validation_data, validation_records, config, device
            )
            scheduler.step()
            history.append(
                {
    ...
    (root / "training-history.json").write_text(
        json.dumps(
            {
                "result": asdict(result),
                "augmentation": {
                    "p_noise": config.p_noise,
                    "clean_noisy_ratio": [1 - config.p_noise, config.p_noise],
                    "families": config.train_noise_families,
                    "snr_policy": "Uniform continuous keyed by (seed, epoch, sample_id)",
                },
                "history": history,
            },
            indent=2,
