...
def stft_magnitude(
    waveform: Tensor, *, n_fft: int = 512, hop_length: int = 160
) -> Tensor:
...
def welch_psd(
    waveform: Tensor, *, sample_rate: int, nperseg: int = 512
) -> tuple[np.ndarray, np.ndarray]:
    _mono(waveform)
    frequencies, power = welch(
        waveform.detach().cpu().numpy(),
        fs=sample_rate,
        nperseg=min(nperseg, waveform.numel()),
    )
...
def mfcc_features(
    waveform: Tensor, *, sample_rate: int, n_mfcc: int = 40, n_mels: int = 80
) -> Tensor:
    _mono(waveform)
...
def mfcc_summary_embedding(
    waveform: Tensor, *, sample_rate: int, n_mfcc: int = 40, n_mels: int = 80
) -> Tensor:
    """Mean/std MFCC utterance vector, L2-normalised for cosine verification."""
    features = mfcc_features(
        waveform, sample_rate=sample_rate, n_mfcc=n_mfcc, n_mels=n_mels
    )
...
def residual_snr_db(
    clean: Tensor, processed: Tensor, *, epsilon: float = 1e-8
) -> float:
    _mono(clean)
    _mono(processed)
    ...
    return float(
        10
        * torch.log10(
            clean.square().mean().clamp_min(epsilon)
            / (processed - clean).square().mean().clamp_min(epsilon)
        )
    )
