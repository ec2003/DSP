...
def test_dsp_features_have_documented_shapes_and_are_finite() -> None:
    ...
    assert (
        torch.isfinite(stft).all()
        and torch.isfinite(mfcc).all()
        and torch.isclose(embedding.norm(), torch.tensor(1.0))
    )
...
def test_paired_bootstrap_is_deterministic() -> None:
    labels = np.asarray([1, 1, 0, 0])
    raw = np.asarray([0.7, 0.4, 0.6, 0.2])
    dsp = np.asarray([0.9, 0.8, 0.3, 0.1])
    first = paired_stratified_bootstrap(
        raw, dsp, labels, threshold_raw=0.5, threshold_dsp=0.5, repetitions=100, seed=9
    )
...
def test_new_conditions_declare_required_ablation_stages() -> None:
    ...
    assert ExperimentConfig(condition="high_pass_low_pass").stages == (
        "high_pass",
        "low_pass",
    )
    assert ExperimentConfig(condition="full_dsp").stages == (
        "high_pass",
        "low_pass",
        "wiener",
    )
