...
from src.orchestration import (
    ConfigMismatchError, PHASE_HANDLERS, PreflightError, create_run, execute_phase,
    next_run_id, open_run, preflight_dataset,
)


def tiny_config_path(tmp_path: Path) -> Path:
    payload = json.loads(Path("configs/config.json").read_text())
    payload.update({"output_root": str(tmp_path / "outputs"), "vctk_root": str(tmp_path / "wav48"), "musan_root": str(tmp_path / "musan"), "train_speakers": 1, "validation_speakers": 1, "clips_per_speaker": 1, "eval_clips_per_speaker": 2, "enrollment_clips": 1, "closed_set_clips": 0, "noise_pool_size": 12})
...
def dataset_layout(config) -> None:
    ...
    for family in ("noise", "music", "speech"):
        for index in range(12):
            path = Path(config.musan_root) / family / "source-set" / f"recording-{index}.wav"
            path.parent.mkdir(parents=True, exist_ok=True); path.touch()


def test_preflight_accepts_full_layout_and_rejects_missing_family(tmp_path):
    path = tiny_config_path(tmp_path); config = load_config(path); dataset_layout(config)
    assert preflight_dataset(config)["musan"]["speech"]["test"] > 0
    for item in Path(config.musan_root).joinpath("music").rglob("*.wav"): item.unlink()
...
def test_run_allocation_snapshot_and_source_mismatch(tmp_path):
    ...
    payload = json.loads(path.read_text()); payload["epochs"] += 1; path.write_text(json.dumps(payload))
...
def test_failure_resume_and_completed_immutability(tmp_path, monkeypatch):
    ...
    def fail(config, workers):
        calls.append("prepare"); raise RuntimeError("stop")
    ...
    def finish(config, workers):
        calls.append("prepare"); return {"ok": True}
    monkeypatch.setitem(PHASE_HANDLERS, "prepare", finish)
    assert execute_phase(context, "prepare").data == {"ok": True}
    assert json.loads(context.config.manifest_path.read_text())["phases"]["prepare"]["attempts"] == 2
    with pytest.raises(RuntimeError, match="requires completed"):
        execute_phase(context, "evaluate_dsp")
    manifest = json.loads(context.config.manifest_path.read_text()); manifest["status"] = "completed"
...
def test_evaluation_handlers_route_only_their_intended_cells(tmp_path, monkeypatch):
    context = create_run(tiny_config_path(tmp_path), project_root=Path.cwd())
    routed = []
    monkeypatch.setattr("src.study.evaluate_study", lambda config, *, workers, cells: routed.append(cells) or [])
    monkeypatch.setattr("src.study.evaluation_summary", lambda config, cells, label: {"rows": [], "label": label})
...
def test_dsp_phase_resumes_without_rerunning_completed_dependencies(tmp_path, monkeypatch):
    context = create_run(tiny_config_path(tmp_path), project_root=Path.cwd())
    manifest = json.loads(context.config.manifest_path.read_text())
    for phase in ("prepare", "eda", "train", "evaluate_raw"):
        manifest["phases"][phase]["status"] = "completed"
    context.config.manifest_path.write_text(json.dumps(manifest))
    calls = []
    monkeypatch.setitem(PHASE_HANDLERS, "evaluate_dsp", lambda config, workers: calls.append("evaluate_dsp") or {"reports": []})
...
def test_notebook_phase_cells_execute_independently_with_mock_backend(tmp_path):
    ...
    def fake_phase(context, phase, *, workers):
        calls.append(phase)
        marker = tmp_path / f"{phase}.json"; marker.write_text("persisted")
        payloads = {
            "prepare": {"inventory": {}}, "eda": {"design": {"selected_edges_hz": {}, "retained_energy_pct": {}}, "figure": "eda.png"},
            "train": {"training": [], "checkpoints": [], "figures": []},
            "evaluate_raw": {"reports": [], "summary": {"rows": [], "error_samples": []}},
            "evaluate_dsp": {"reports": [], "summary": {"rows": [], "error_samples": []}},
            "signal_analysis": {"positive_control": {}, "characterisation": {}}, "statistics": {"factorial": {}}, "figures": {"figures": []},
        }
        return SimpleNamespace(data=payloads[phase])
    namespace = {"context": object(), "WORKERS": 0, "execute_phase": fake_phase, "display": displayed.append, "Image": lambda **kwargs: kwargs, "config": SimpleNamespace(run_dir=tmp_path, report_root=tmp_path)}
    for cell in notebook["cells"]:
        source = "".join(cell.get("source", []))
        if cell["cell_type"] == "code" and "execute_phase(context" in source:
            exec(source, namespace)
    assert calls == ["prepare", "eda", "train", "evaluate_raw", "evaluate_dsp", "signal_analysis", "statistics", "figures"]
