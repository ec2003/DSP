...
def test_notebook_cli_cells_only_use_supported_run_commands() -> None:
    ...
    loop = re.search(
        r"for name in \((.*?)\):\s+commands\.add_parser\(name\)",
        parser_source,
        flags=re.DOTALL,
    )
    ...
    assert any(
        "full-study" in cell.get("metadata", {}).get("tags", [])
        for cell in notebook["cells"]
    )


def test_notebook_dashboard_reads_saved_artifacts_without_reimplementing_study_logic() -> (
    None
):
    notebook, source = _notebook_source()
    dashboard_source = "\n".join(
...
def test_report_has_render_targets_sections_citations_and_saved_artifact_paths() -> (
    None
):
    ...
    assert (
        "embed-resources: true" in report and "bibliography: references.bib" in report
    )
    for heading in (
